...
# 
# ERRATA: as linhas 181-189, 211-217 e 237-245 agora consideram
# reacalibrar o modelo penalizado depois de fazer a validacao cruzada
# para selecionar o valor do parametro de penalizacao lambda
# 
...
ggplot(data.frame(bic = best_sum$bic, n=1:10), aes(n, bic))+
  geom_point()+
  geom_vline(xintercept = which.min(best_sum$bic), col="red")+
...
best_erro_dentro <- mean((dados_matriz[ids, names(best_coef)] %*% best_coef -
                          Credit$Balance[ids])^2)
best_erro_fora <- mean((dados_matriz[-ids, names(best_coef)] %*% best_coef -
                        Credit$Balance[-ids])^2)

resultados[which(resultados$modelo=="best"), 2:3] = c(best_erro_dentro,
                                                      best_erro_fora)
resultados
...
resultados[which(resultados$modelo=="forward"), 2:3] = c(fw_erro_dentro,
                                                         fw_erro_fora)
resultados

# ---- selecao regressiva (backward stepwise) -----
bw <- regsubsets(Balance ~ ., data = Credit[ids,-1], nvmax = ncol(Credit)-2,
                 method = "backward")

bw_sum <- summary(bw)

ggplot(data.frame(bic=bw_sum$bic, n=1:10), aes(n, bic))+
  geom_point()+
  geom_vline(xintercept = which.min(bw_sum$bic), col="red")+
  scale_x_continuous(breaks=1:10)+
  labs(title = "BIC - backward", x = "Número de variáveis",
       y = "BIC")
...
# calculo do erro estimado dentro e fora da amostra
bw_erro_dentro <- mean((dados_matriz[ids, names(bw_coef)] %*% bw_coef -
                          Credit$Balance[ids])^2)
bw_erro_fora <- mean((dados_matriz[-ids, names(bw_coef)] %*% bw_coef -
                        Credit$Balance[-ids])^2)

resultados[which(resultados$modelo=="backward"), 2:3] = c(bw_erro_dentro,
                                                          bw_erro_fora)
resultados
...
ridge <- glmnet(X[ids,], y[ids], alpha = 0, nlambda = 1000)
...
### INICIO ERRATA ------------
ridge <- glmnet(X[ids,], y[ids], alpha = 0, lambda = cv_ridge$lambda.1se)
...
lasso <- glmnet(X[ids,], y[ids], alpha = 1, nlambda = 1000)
...
### INICIO ERRATA ------------
lasso <- glmnet(X[ids,], y[ids], alpha = 1, lambda = cv_lasso$lambda.1se)
...
# elastic-net -------------------------------------------------------------
elastic <- glmnet(X[ids,], y[ids], alpha = 0.75, nlambda = 500)
...
### INICIO ERRATA ------------
elastic <- glmnet(X[ids,], y[ids], alpha = 0.75, lambda = cv_elastic$lambda.1se)
