...
library(magrittr) # contem o operador pipe %>%
...
modelo_rn <- keras_model_sequential() %>%
  layer_dense(units = 50, # numero de unidades
...
history <- modelo_rn %>%
  fit(x[-ids_teste,], # preditoras de treino
      y[-ids_teste],  # resposta de treino
      batch_size = 32, # observacoes escolhidas aleatoriamentes
